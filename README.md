...
- [x] AWS route53를 사용하여 SSL 인증서 등록 해보기 (유료 실습)
  ...
  - SSL 인증서를 등록하기 위해서는 도메인이 필요함
     
  - AWS의 ROUTE 53 서비스를 통해 도메인을 발급받을 수 있음 (유료)
     
  - 도메인을 발급 받았다면 EC2 탭에서 로드밸런싱 메뉴를 통해 ALB(어플리케이션 로드밸런서)를 생성
     
  - ALB는 외부 도메인은 지원하지 않기 때문에 ROUTE 53을 통해 생성한 도메인만 사용할 수 있음
     
  - ALB를 생성 설정 중 타겟그룹이란 ALB와 통신하는 EC2 그룹들을 의미함. 따라서 타겟그룹과는 프로토콜은 HTTP로 설정해도 무방
     
  - Secure listener settings가 SSL 인증서를 적용하는 부분
     
  - 인증서를 생성할 때 도메인 정보를 넣고 생성 후 인증서를 클릭하고 Route 53에 DNS 레코드를 생성

  - 이후 ALB 생성 완료 후 Route 53 호스팅 영역탭에서 도메인 선택 후 레코드 생성을 통해 도메인이 호출되면 ALB로 넘어가도록 설정
     
  - 위와 같은 방식을 통해 SSL 적용을 할 수 있으며, 추가적으로 HTML 파일안에 http로 jquery나 google icon을 받는 코드가 있다면 오류가 날 수 있음
